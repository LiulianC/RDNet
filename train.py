...
datadir_nature = join(datadir, 'nature20')
...
eval_dataset_real = datasets.DSRTestDataset(join(datadir, 'real20'),
                                            fns=read_fns('data/real_test.txt'), if_align=opt.if_align)

# eval_dataset_solidobject = datasets.DSRTestDataset(join(datadir, 'test/SIR2/SolidObjectDataset'),
#                                                    if_align=opt.if_align)
eval_dataset_solidobject = datasets.DSRTestDataset(join(datadir, 'SIR2/SolidObjectDataset'),if_align=opt.if_align)

eval_dataset_postcard = datasets.DSRTestDataset(join(datadir, 'SIR2/PostcardDataset'), if_align=opt.if_align)
eval_dataset_wild = datasets.DSRTestDataset(join(datadir, 'SIR2/WildSceneDataset'), if_align=opt.if_align)
...
result_dir = os.path.join(f'./experiment/{opt.name}/results',mutils.get_formatted_time())
